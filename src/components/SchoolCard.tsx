    ...
    <Card className="border-0 shadow-sm d-flex flex-md-row my-2">
      ...
      <Card.Body className="d-flex flex-column align-items-center">
        ...
        <ul className="d-flex flex-wrap flex-md-row gap-2 list-unstyled ps-0 card-tags">
          {school.tags.map((tag: any, index: number) => (
            <li key={index}>
              {/* Placeholder for tag links */}
            </li>
          ))}
        </ul>
        ...
        {(roles && privileged_school_roles.some(role => roles.includes(role))) && (
          <Row className="d-flex flex-row mt-5">
            <span>
              ...
              {validUser && (
                <Card.Link className="text-decoration-none" onClick={handleDelete}>
                  <em>Delete</em>
                </Card.Link>
              )}
              <Card.Link className="text-decoration-none" onClick={handleDetails}>
                <em>Details</em>
              </Card.Link>
            </span>
          </Row>
