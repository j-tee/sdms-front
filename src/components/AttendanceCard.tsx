...
const AttendanceCard = (props: any) => {
  ...
  const [attendeeParams, setAttendeeParams] = useState({
    ...
    lesson_id: 0,
    program_id: 0,
    attendance_date: '', //new Date().toISOString().split('T')[0], // Default to today's date
    branch_id: branchId,
    school_id: schoolId,
    class_group_id: 0,
    paginate: true,
  ...
  const isAllPresent = onRoll.every((student) => student.status === 'present');
    ...
    <>
            ...
            <thead>
              <tr>
                ...
                <Form.Check
                    ...
                    label={isAllPresent ? 'All Present' : 'Mark All Present'}
                  />
                </th>
              </tr>
            </thead>
      ...
      <div className="d-flex flex-column flex-md-row px-2 justify-content-between align-items-center">
        ...
        >
          ...
          <Dropdown.Item onClick={() => handleAttendeesItemsPerPageChange(100)}>100</Dropdown.Item>
          <Dropdown.Item onClick={() => handleAttendeesItemsPerPageChange(200)}>200</Dropdown.Item>
          <Dropdown.Item onClick={() => handleAttendeesItemsPerPageChange(500)}>500</Dropdown.Item>
          <Dropdown.Item onClick={() => handleAttendeesItemsPerPageChange(1000)}>1000</Dropdown.Item>
        </DropdownButton>
      </div>
    </>
  );
